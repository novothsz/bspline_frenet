            ...
            for i, key in enumerate(obstacle_positions):
                ...
                if key == "obst0":
                 ...
                 # Rest of the obstacles :)        
                    ...
                    obstacle = Obstacle(ID = i+2, corners = corners,obstacle_type="pole") # Randomly placed obstacle
                    obstacles += [obstacle]
                    
                    
            # Poles randomly placed
            num_of_random_obstacles = 1
            if len(obstacles) > 0:
                obst_ID = int(max(obstacle_positions.keys())[-1])+2
                # obst_ID = obstacles[-1].ID + 1
            else:
                obst_ID = 0
                
            for i in range(num_of_random_obstacles):
                obstacle = Obstacle(ID = obst_ID, obstacle_type="pole") # Randomly placed obstacle
                obstacles += [obstacle]
                obst_ID += 1
