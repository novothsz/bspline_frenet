...
class Flipper():
    def __init__(self):
        ...
        Xk = self.q0 + self.omega0
        Uk = 1e-2 * 1 # self.DvX.tau[0]
        kappa = True
        state_history = []
        for i in range(self.N):
            ...
            if self.CVODES == True:
                ...
                intQ = res['Q']
            Uk = 0
            state_history += [Xk.full().reshape(1, -1).tolist()]
                
        
        
        
        
        state_history = np.array(state_history).reshape(-1, 3)
        
        
        figs, axs = plt.subplots(2, 3)
        for i in range(2):
            # Plotting
            axs[0, 2].plot(state_history[:, i])
        axs[0, 2].plot(state_history[:, 0]**2 + state_history[:, 1]**2)
        axs[0, 2].plot([1] * state_history.shape[0], 'k:' )
    ...
    def setup_solver(self):
        ...
        for i in range(N):
            ...
            J = J + 1.0 * intQ    
            if i == N - 1:
                g += [Xk_end]; lbg += [self.qf, self.omegaf]; ubg += [self.qf, self.omegaf]
                # J = J + 1.0 * (Xk_end[0] - self.qf[0])**2 + 1.0 * (Xk_end[1] - self.qf[1])**2
            else:    
                g += [Xk_end]; lbg += [self.q_min, self.omega_min]; ubg += [self.q_max, self.omega_max]
